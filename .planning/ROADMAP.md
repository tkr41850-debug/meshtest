...
**Success criteria:**
- [ ] Status calculation accepts data arriving up to 120s after the expected interval without marking "NotAvailable"
- [ ] Existing OK/Pending/NotAvailable status values are preserved for genuinely stale data
- [ ] All existing tests pass (no regression in status logic)

### Phase 6: Frontend — Dashboard Bug Fix + UptimeRobot-style Details
...
**Success criteria:**
- [ ] Dashboard auto-refreshes without `StreamlitAPIException`
- [ ] Each node-pair expander shows an UptimeRobot-style summary card with: status badge, uptime %, latest ping/HTTP latency, last check timestamp
- [ ] Cards are visually distinct from the current flat table layout
- [ ] All existing dashboard views (30m matrix, 30d view) remain functional
