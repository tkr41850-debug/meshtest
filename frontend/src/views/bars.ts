...
export function renderBars(bars: BarEntry[]): string {
  return bars
    .map((b) => {
      ...
      return `<span data-history-bar style="display:inline-block;width:8px;height:20px;border-radius:1px;background:${uptimeColor(b.percent)};border:1px solid #d1d5db;" title="${title}"></span>`;
    })
    .join("");
}
